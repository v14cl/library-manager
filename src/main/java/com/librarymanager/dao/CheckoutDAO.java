...
public class CheckoutDAO implements CrudDAO<Checkout> {
    ...
    @Override
    public Checkout findById(int id) throws SQLException {
        Validator.validateId(id, "Checkout ID");
    ...
    @Override
    public void update(Checkout ch) throws SQLException {
        Validator.validateId(ch.getCheckoutId(), "Checkout ID");

        String sql = "UPDATE checkout SET client_id = ?, book_id = ?, date_taken = ?, deadline = ?, date_returned = ? " +
                     "WHERE checkout_id = ?";

        try (Connection c = DatabaseConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            fillStatement(ps, ch);
            ps.setInt(6, ch.getCheckoutId());
            ps.executeUpdate();
        }
    }
    ...
    public void markReturned(int checkoutId, LocalDate returnedDate) throws SQLException {
        if (returnedDate == null) returnedDate = LocalDate.now();
    ...
    private void validateCheckout(Checkout ch) throws SQLException {
        ...
        if (countActiveCheckouts(ch.getClientId()) >= 5)
            throw new IllegalStateException("Client can't have more than 5 books.");
    }

    private void prepareDefaults(Checkout ch) {
        if (ch.getDateTaken() == null) ch.setDateTaken(LocalDate.now());
        if (ch.getDeadline() == null) ch.setDeadline(ch.getDateTaken().plusDays(30));
    }
    ...
    private void fillStatement(PreparedStatement ps, Checkout ch) throws SQLException {
        ...
        if (ch.getDateReturned() != null) ps.setDate(5, Date.valueOf(ch.getDateReturned()));
